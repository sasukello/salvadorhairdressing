...
function progUno(){ //PROGRAMAR VISITA
        ...
        $sql = "SELECT A.id as iden, A.nombre, A.apellido, A.pais, A.estado, A.ciudad, A.nacimiento, coalesce(B.visita_asistencia, '-1') as asist FROM `ms_usuario` A LEFT JOIN ms_usuario_visitas B on A.id = B.id_usuario WHERE A.`status` = 1 ORDER BY A.`pais` ASC";
        $search = mysqli_query($dbh, $sql);
        $match = mysqli_num_rows($search);
        if ($match > 0) {
            echo "<div class='table-responsive'>
            ...
            while ($rw = mysqli_fetch_array($search)) {
                ...
                echo "<tr>
                        <td>";
                        ?><a class='btn btn-link' href=<?php echo 'visitaProgramar.php?p='.$idsecret.'&r='.$paissecret.'';?> onclick="window.open(this.href,'targetWindow','toolbar=no,location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=900, height=560, top=50, left=250'); return false;"><?php echo "$nombre $apellido</a>"; if(isset($status)){echo $status;} echo "</td>
                        <td>"?><a class='btn btn-link' href=<?php echo 'visitaProgramar.php?p='.$idsecret.'&r='.$paissecret.'';?> onclick="window.open(this.href,'targetWindow','toolbar=no,location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=900, height=560, top=50, left=250'); return false;"><?php echo "$edadf</td>
                        <td>$paises[$pais]</td>
                        <td>$estado, $ciudad</td>";?>
