...
function ventatotal($desde, $hasta){
        ...
        $sql = "select Neto, MontoIva as MONTOIMPUESTO, Total, Efec as EFECTIVO, TDB as TARJETADEBITO, TDC as TAARJETACREDITO, CHE as CHEQUE, CRE as CREDITO, Propinas as PROPINA, efectivocxc as cxcefectivo, chequecxc as cxccheque  from 
(Select ventas.Clave, Neto, MontoIva, Total, Efec, TDB, TDC, CHE, CRE, Propinas
from ( 
       select '1' as clave,
        Sum(iif(TIPO = 'FAC', Total - MontoIva, (Total - MontoIva) * -1)) as Neto,
        Sum(iif(TIPO = 'FAC', MontoIva, MontoIva * -1)) as MontoIva,
        Sum(iif(TIPO = 'FAC', Total, total * -1)) as Total,
        Sum(iif(TIPO = 'FAC', Efectivo, Efectivo * -1)) as EFEC,
        Sum(iif(TIPO = 'FAC', TDebito, tDebito * -1)) as TDB,
        Sum(iif(TIPO = 'FAC', TCredito, tCredito * -1)) as TDC,
        Sum(iif(TIPO = 'FAC', Cheque, Cheque * -1)) as CHE,
        Sum(iif(TIPO = 'FAC', Credito, Credito * -1)) as CRE
from  vta
where tipo in ('FAC', 'DEV') and
      fechaventa >= $desde and
      fechaventa <= $hasta) ventas
      
left join

(select '1' as clave, sum(monto) as Propinas from VENTASPROPINAS where correlativovta in (
select correlativo from vta where fechaventa >= $desde and fechaventa <= $hasta and tipo = 'FAC') and tipodocumento = 'FAC') propina

on ventas.clave = propina.clave) totales
...
(select '1' as clave,
 ...
 from cxc 
 where fechaingreso >= $desde and FechaIngreso <= $hasta) ccd

on totales.clave = ccd.clave";
        ...
        $sql = "Select
                       ...
                       Sum(case vtadte.Tipo when 'DEV' then
                                ...
                                (((vtadte.PrecioSinIva -  vtadte.descuento - vtadte.descuentopuntos) * vtadte.Cantidad) -

                                (((vtadte.PrecioSinIva -  vtadte.descuento - vtadte.descuentopuntos) * vtadte.Cantidad) *

                                (((vta.descuento * 100) / (vta.SubTotal - vta.desclineas - coalesce(vta.MontoRedimido, 0))) / 100)) ) * -1
                                ...
                                (((vta.descuento * 100) / (vta.SubTotal - vta.desclineas  - coalesce(vta.MontoRedimido, 0))) / 100)) end) as Servicios
...
function ventadia($desde, $hasta, $filtro){
        ...
        if ($filtro == "A"){
            ...
            $sql = "Select CCD.FechaVenta, Efec as Efectivo, CHE as Cheque, TDB as TDebito, TDC as TCredito, CRE as Credito, Subtotal, Descuento, DescLineas, Neto as MontoNeto, MontoIva, Total, PRO as TPropina  from
(Select
   FechaVenta,
   Neto,
   MontoIva,
   SubTotal, Descuento, DescLineas,
   Total,
   Efec,
   Tdb,
   Tdc,
...
from
   (select
      ...
      F.subtotal - Coalesce(D.Subtotal, 0) as SubTotal,
      F.descuento - Coalesce(D.descuento, 0) as Descuento,
      F.desclineas - Coalesce(D.desclineas, 0) as Desclineas,
   ...
   from
      (Select
         ...
         sum(DescLineas + Coalesce(MontoRedimido, 0)) as DescLineas,
...
function clientesatendidos($desde, $hasta, $asociados){ 
  ...
  $arsql = array();
  //Para el detalle diario de comisiones
  $sql="select clientesatendidos.*,
       (Select listaasociados from  (select clientecodigo, list(distinct('''' || asociado || ''''), ',') as listaasociados from vta v11, vtadte vd11 where v11.CORRELATIVO = vd11.CORRELATIVOPRINCIPAL and v11.tipo = vd11.tipo and v11.fechaventa = clientesatendidos.ultimavisita and vd11.PORCCOM <> 0 group by clientecodigo) a where a.clientecodigo = clientesatendidos.clientecodigo) as Asociados       
       
from 
(select vd.asociado as CODIGO,
       (Select Nombre from asociados a  where a.codigo = vd.asociado) as Nombre,
       (Select Estatus from asociados a  where a.codigo = vd.asociado) as Estatus,
       v.clientecodigo,
       ...
       sum(iif(vd.tipo = 'DEV', cantidad * -1, cantidad)) as Realizados,
       (Select max(fechaventa) from vta vv where vv.tipo = 'FAC' and vv.CLIENTECODIGO = V.CLIENTECODIGO) as UltimaVisita
from vta v, vtadte vd
where v.correlativo = vd.correlativoprincipal and v.tipo = vd.tipo and
      v.fechaventa >= ".$desde." and
      v.fechaventa <= ".$hasta." and
      vd.asociado in (".$asociados.") and
      v.SUBTOTAL > 0
group by vd.asociado,
       v.clientecodigo,
       V.clientenombre
order by Asociado, Realizados DESC) clientesatendidos";
...
/******* SALVADOR PLUS LIVE: OPERACIONES *************/

function productosactuales($linea){
  $arsql = array();
  $sql   = "Select Codigo, Descripcion, Marca, Linea, Costounitario, Ultimocosto, Alicuotacompra, Alicuotaventa, Precioventa1,Precioventa2,Precioventa3,Precioventa4,Porcutilidad1,Porcutilidad2,Porcutilidad3,Porcutilidad4, Estatus, Tipoventa, Tipo, Preciosiniva1, Preciosiniva2,Preciosiniva3,Preciosiniva4, Descontinuado, (Select first 1 C.PORCMAXIMOUTILIDAD from Configuracion C) as UtilidadMaxima, (Select first 1 C.PORCMINIMOUTILIDAD from Configuracion C) as UtilidadMinima from Productos WHERE Linea in (".$linea.") AND Tipo = 'P' order by Codigo ASC";
...
function serviciosactuales(){
  $arsql = array();
  $sql   = "Select Codigo, Descripcion, Alicuotacompra, Alicuotaventa, Precioventa1,Precioventa2, Fechacreacion, Usuariocreado, Estatus, Tipoventa, Tipo, Preciosiniva1, Preciosiniva2, Descontinuado from Productos WHERE Tipo = 'S' order by Codigo ASC";
  $arsql[] = $sql;
  return urlsafe_b64encode(serialize($arsql));
}

function productosmarcas(){
  $arsql = array();
  $sql   = "select a.codigo as codmarca, a.nombre as nommarca from marca a";

  $arsql[] = $sql;
  return urlsafe_b64encode(serialize($arsql));
}

function productoslineas($marcas){
  $arsql = array();
  $sql   = "select codigo, nombre from linea where codigomarca in (".$marcas.")";

  $arsql[] = $sql;
  return urlsafe_b64encode(serialize($arsql));
}

function saveProductEdit($dataset, $user){
  $arsql = array();
  $set = array();
  $set = (array) json_decode($dataset, true);
  ...
  //DESCRIPCION = '', AUN FALTA EDITAR LA DESCRIPCION  

  $sql = "UPDATE productos SET COSTOUNITARIO = ".$set["cu"].", ULTIMOCOSTO = ".$set["cuant"].", ALICUOTACOMPRA = ".$set["acompra"].", ALICUOTAVENTA = ".$set["aventa"].", PRECIOVENTA1 = ".round($set["pv1"],2).", PRECIOVENTA2 = ".round($set["pv2"],2).", PORCUTILIDAD1 = ".$set["ut1"].", PORCUTILIDAD2 = ".$set["ut2"].", FECHAMODIFICACION = CURRENT_DATE, USUARIOMODIFICADO = '".$user."', TIPOVENTA = '".$set["tvent"]."', COSTOPROMEDIO = ".$costpromed.", PRECIOSINIVA1 = ".$set["psi1"].", PRECIOSINIVA2 = ".$set["psi2"].", DESCONTINUADO = '".$descont."' where CODIGO = '".$set["codp"]."';";
...
function createService($dataset, $user){
  $arsql = array();
  $set = array();
  $set = (array) json_decode($dataset, true);

  $sql = "INSERT INTO productos (CODIGO, DESCRIPCION, ALICUOTACOMPRA, ALICUOTAVENTA, PRECIOVENTA1, PRECIOVENTA2, PRECIOVENTA3, PRECIOVENTA4, PRECIOSINIVA1, PRECIOSINIVA2, PRECIOSINIVA3, PRECIOSINIVA4, ESTATUS, FECHACREACION, USUARIOCREADO, TIPOVENTA, DESCONTINUADO, PORCCLIENTCARD, TIPO) VALUES ('".$set["codigo"]."', '".$set["descripcion"]."', ".$set["alic"].", ".$set["alicv"].", ".round($set["pv1"],2).", ".round($set["pv2"],2).", ".round($set["pv3"],2).", ".round($set["pv4"],2).", ".$set["psi1"].", ".$set["psi2"].", ".$set["psi3"].", ".$set["psi4"].", 'A', CURRENT_DATE, '".$user."', 'R', 'F', 5, 'S' )";
...
function createProduct($dataset, $user){
  $arsql = array();
  $set = array();
  $set = (array) json_decode($dataset, true);

  $sql = "INSERT INTO productos (CODIGO, DESCRIPCION, PORCUTILIDAD1, PORCUTILIDAD2, COSTOUNITARIO, COSTOPROMEDIO, ALICUOTACOMPRA, ALICUOTAVENTA, PRECIOVENTA1, PRECIOVENTA2, PRECIOSINIVA1, PRECIOSINIVA2, LINEA, MARCA, ESTATUS, FECHACREACION, USUARIOCREADO, TIPOVENTA, DESCONTINUADO, PORCCLIENTCARD, TIPO) VALUES ('".$set["codigo"]."', '".$set["descripcion"]."', ".$set["ut1"].", ".$set["ut2"].", ".$set["cu"]." , 0, ".$set["alic"].", ".$set["alicv"].", ".round($set["pv1"],2).", ".round($set["pv2"],2).", ".$set["psi1"].", ".$set["psi2"].", ".$set["linea"].", ".$set["marca"].", '".$set["estado"]."', CURRENT_DATE, '".$user."', '".$set["tvent"]."', 'F', 5, 'P' )";
...
/******* CORPORATIVO INTRANET: RRHH *************/

function listaasociados(){

  $arsql = array();  

  $sql   = "select codigo, nombre from asociados where estatus = 'A' and codigo not in ('00', '20') order by Nombre ";
...
function consultadesbloquear($asociado){

   $arsql = array();  

  $sql   = "Update Asociados set Desbloquear = 'V' where codigo = '". $asociado ."'";
...
/*  $desde = "'".$desde."'";
  ...
  $arsql = array();
  $sql   = "
  Select * From ORDENES_PAGOS_CUENTAS";
  $arsql[] = $sql;
return urlsafe_b64encode(serialize($arsql));
}
